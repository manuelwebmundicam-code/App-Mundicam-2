...
class ApiService {
      ...
      final response = await _appGet(
        ...
        queryParameters: <String, dynamic>{
          ...
          if ((brandName ?? '').trim().isNotEmpty &&
              _sendBrandNameAlongsideId(brandId, brandName!.trim()))
  ...
  Future<int?> getMarcaIdPorNombre(String? brandName) async {
    ...
    final marcas = await getMarcas(hideEmpty: false);
  ...
  bool _sendBrandNameAlongsideId(int? brandId, String brandName) {
    if (brandId == null || brandId <= 0) return brandName.trim().isNotEmpty;

    // Compatibilidad con el backend actual: algunas versiones históricas del
    // plugin convierten el texto "MCI" en "MCI PRO" antes de usar brand_id.
    // Si ya tenemos el ID exacto de MCI, enviamos solo ese ID para respetar la
    // marca real seleccionada. MCI PRO sí puede enviar nombre + ID.
    return _canonicalBrandKey(brandName) != 'mci';
  }
  ...
  Future<CatalogProductsResult> _requestCatalogProductsPage({
    ...
    final params = <String, dynamic>{
      ...
      if (brandName.isNotEmpty && _sendBrandNameAlongsideId(brandId, brandName))
        'brand': brandName,
      if (brandName.isNotEmpty && _sendBrandNameAlongsideId(brandId, brandName))
  ...
  String? _knownBrandInSearch(String query) {
    ...
    // MCI y MCI PRO son marcas independientes y nunca se resuelven entre sí.
  ...
  static bool _brandKeysEquivalent(String left, String right) {
    ...
    // MCI y MCI PRO son dos marcas distintas. Sin esta protección, la
    // comparación por inclusión posterior haría que "mcipro" contuviese "mci".
    if ((a == 'mci' || a == 'mcipro') ||
        (b == 'mci' || b == 'mcipro')) {
      return false;
    }
