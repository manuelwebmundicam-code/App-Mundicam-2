...
class _ProfilePageState extends ConsumerState<ProfilePage> {
    ...
    try {
      ...
      if (userDoc.exists) {
        ...
        if (firestoreData?['isBlocked'] == true) {
          if (mounted) setState(() { _errorMessage = 'Cuenta bloqueada'; _loadingData = false; });
          return;
        }
      ...
      if (email == null || email.isEmpty) {
        if (mounted) setState(() { _errorMessage = 'Email no disponible. Contacta con soporte.'; _loadingData = false; });
        return;
      }
      ...
      if (wooCustomer != null) {
        ...
        if (mounted) setState(() { _wooCustomer = wooCustomer; _loadingData = false; });
      } else {
        debugPrint('⚠️ Cliente no encontrado en WooCommerce');
        if (mounted) setState(() { _errorMessage = 'Cliente no encontrado.\nContacta con tu gestor comercial.'; _loadingData = false; });
      }
    } catch (e) {
      debugPrint('❌ Error: $e');
      if (mounted) setState(() { _errorMessage = 'Error de conexión'; _loadingData = false; });
    }
  }
  ...
  String _getCompany() => _wooCustomer?['billing']?['company']?.toString() ?? '';
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text("MI CUENTA"), centerTitle: true, elevation: 0,
      ...
      body: user == null ? const Center(child: Text("No has iniciado sesión"))
          : _loadingData ? const Center(child: CircularProgressIndicator(color: AppColors.primary))
          : SingleChildScrollView(physics: const ClampingScrollPhysics(), child: Column(children: [
        _buildHeader(),
        Padding(padding: const EdgeInsets.all(16), child: Column(children: [
          _buildMainCard(), const SizedBox(height: 24),
          Row(children: [_quickButton(context, Icons.request_quote_outlined, "Presupuestos", const QuotesPage()), const SizedBox(width: 12), _quickButton(context, Icons.local_shipping_outlined, "Mis Pedidos", const OrdersPage())]),
  ...
  Widget _buildHeader() {
    return Container(
      ...
      child: SafeArea(bottom: false, child: Padding(padding: const EdgeInsets.fromLTRB(24, 8, 24, 32), child: Column(children: [
        Stack(children: [
          Container(width: 80, height: 80, decoration: BoxDecoration(color: Colors.white.withOpacity(0.2), shape: BoxShape.circle, border: Border.all(color: _isAdmin ? Colors.amber : Colors.white.withOpacity(0.3), width: 3)), child: Center(child: Text(_getInicial(), style: const TextStyle(color: Colors.white, fontSize: 36, fontWeight: FontWeight.w800, fontFamily: 'Oswald')))),
        ...
        if (_getCompany().isNotEmpty) ...[const SizedBox(height: 4), Text(_getCompany(), textAlign: TextAlign.center, style: TextStyle(color: Colors.white.withOpacity(0.9), fontSize: 14))],
        const SizedBox(height: 8),
        Text(_wooCustomer?['email'] ?? '', style: TextStyle(color: Colors.white.withOpacity(0.8), fontSize: 14)),
        if (_isAdmin) ...[const SizedBox(height: 8), Container(padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4), decoration: BoxDecoration(color: Colors.amber.withOpacity(0.2), borderRadius: BorderRadius.circular(12), border: Border.all(color: Colors.amber.withOpacity(0.3))), child: const Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.admin_panel_settings, color: Colors.amber, size: 16), SizedBox(width: 6), Text("ADMINISTRADOR", style: TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold, letterSpacing: 0.5))]))],
      ]))),
    );
  }

  Widget _buildMainCard() {
    if (_errorMessage != null) return Container(width: double.infinity, padding: const EdgeInsets.all(20), decoration: _cardDecoration(), child: Column(children: [const Icon(Icons.error_outline, size: 48, color: Colors.orange), const SizedBox(height: 12), Text(_errorMessage!, textAlign: TextAlign.center, style: const TextStyle(fontSize: 14, color: Colors.grey)), const SizedBox(height: 16), ElevatedButton.icon(onPressed: () { setState(() { _loadingData = true; _errorMessage = null; }); _cargarDatosUsuario(); }, icon: const Icon(Icons.refresh), label: const Text("Reintentar"), style: ElevatedButton.styleFrom(backgroundColor: AppColors.primary, foregroundColor: Colors.white))]));
    ...
    return Container(width: double.infinity, padding: const EdgeInsets.all(16), decoration: _cardDecoration(), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const Divider(height: 20), _infoRow(Icons.badge_outlined, "CIF / NIF", _getMeta('cif_nif')),
      const Divider(height: 20), _infoRow(Icons.support_agent_outlined, "Gestor asignado", _getMeta('assigned_manager')),
  ...
  BoxDecoration _cardDecoration() => BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: [BoxShadow(color: Colors.black.withOpacity(0.04), blurRadius: 12, offset: const Offset(0, 4))]);
