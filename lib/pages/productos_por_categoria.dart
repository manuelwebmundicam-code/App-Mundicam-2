...
class _ProductosPorCategoriaScreenState
  ...
  void _onScrollThrottled() {
    ...
    _scrollTimer = Timer(const Duration(milliseconds: 100), () {
      if (_scrollController.position.pixels >=
        ...
        final notifier = ref.read(productsPaginatedProvider(widget.categoryId).notifier);
  ...
  Widget build(BuildContext context) {
    ...
    final productosState = ref.watch(productsPaginatedProvider(widget.categoryId));
    final notifier = ref.watch(productsPaginatedProvider(widget.categoryId).notifier);
...
class _ProductTileState extends ConsumerState<ProductTile> {
            ...
            InkWell(
              onTap: () {
                Navigator.push(
                  context,
                  MaterialPageRoute(
                    builder: (context) => ProductDetailScreen(product: widget.p),
                  ),
                );
              },
              child: Row(
                ...
                children: [
                  ...
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text(widget.p.name, maxLines: 2, overflow: TextOverflow.ellipsis,
                        ...
                        Text(
                          widget.p.shortDescription ?? "Sin descripción disponible",
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: _puedeComprar
                        ? () {
                      ref.read(cartProvider.notifier).addProduct(widget.p, cantidad);
                ...
                child: Opacity(
                  ...
                  child: Container(
                    ...
                    child: Row(
                      ...
                      children: [
                        _qtyBtn(Icons.remove, _tieneStock, () { if (cantidad > 1) setState(() => cantidad--); }),
                        Text("$cantidad",
                            style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: _tieneStock ? Colors.black : Colors.grey)),
                        _qtyBtn(Icons.add, _tieneStock, _aumentarCantidad, isPrimary: _tieneStock),
                      ],
                    ),
                  ),
                ),
                  ...
                  child: OutlinedButton(
                    onPressed: _puedeComprar
                      ...
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(content: Text("${widget.p.name} añadido al carrito"),
                            backgroundColor: AppColors.primary, duration: const Duration(seconds: 1)),
                      );
                    }
                        : null,
                    style: OutlinedButton.styleFrom(
                      side: BorderSide(color: _tieneStock ? AppColors.primary : Colors.grey.shade400, width: 1.3),
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6)),
                      padding: const EdgeInsets.symmetric(vertical: 0),
                    ),
                    child: Text(
                      _tieneStock ? "AÑADIR AL CARRITO" : "SIN STOCK",
                      style: TextStyle(fontSize: 10, color: _tieneStock ? AppColors.primary : Colors.grey.shade400, fontWeight: FontWeight.bold),
                    ),
                  ),
                ),
              ),
              ...
              Expanded(
                child: SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    onPressed: (_tieneStock && !_isAddingToQuote) ? () => _addToQuote(widget.p) : null,
                    icon: _isAddingToQuote
                        ? const SizedBox(width: 14, height: 14, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.orange))
                        : Icon(Icons.description_outlined, size: 14, color: _tieneStock ? Colors.orange.shade700 : Colors.grey.shade400),
                    label: Text(
                      _isAddingToQuote ? "..." : (_tieneStock ? "PRESUPUESTO" : "SIN STOCK"),
                      style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: _tieneStock ? Colors.orange.shade700 : Colors.grey.shade400),
                    ),
                    style: OutlinedButton.styleFrom(
                      foregroundColor: _tieneStock ? Colors.orange.shade700 : Colors.grey.shade400,
                      side: BorderSide(color: _tieneStock ? Colors.orange.shade700 : Colors.grey.shade400, width: 1.3),
                      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(6)),
                      padding: const EdgeInsets.symmetric(vertical: 0),
                    ),
                  ),
  ...
  Widget _qtyBtn(IconData icon, bool enabled, VoidCallback onTap, {bool isPrimary = false}) {
    return GestureDetector(
      onTap: enabled ? onTap : null,
      child: Container(
        width: 30, height: 36,
        alignment: Alignment.center,
        child: Icon(icon, size: 16, color: enabled ? (isPrimary ? AppColors.primary : Colors.black87) : Colors.grey.shade400),
      ),
    );
  }
  ...
  Future<void> _addToQuote(Product product) async {
    ...
    try {
      ...
      if (email == null || email.isEmpty) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text("No se pudo obtener tu email. Contacta con soporte."), backgroundColor: Colors.red, duration: Duration(seconds: 3)),
          );
        }
        return;
      }
      ...
      await apiService.crearPresupuesto(
        ...
        productName: product.name, price: precioDouble, quantity: cantidad,
      );

      if (mounted) {
        ScaffoldMessenger.of(context).clearSnackBars();
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("$cantidad x ${product.name} añadido al presupuesto"), backgroundColor: Colors.green.shade700, duration: const Duration(seconds: 2)),
        );
      }
    ...
    return Container(
      ...
      child: ClipRRect(
        ...
        child: CachedNetworkImage(
          ...
          errorWidget: (_, __, ___) => const Icon(Icons.broken_image, color: Colors.grey, size: 30),
